...
#    get distance between planet and other object
    def calcRadius(self,other):
        deltaX = abs(self.locX-other.locX)
        deltaY = abs(self.locY-other.locY)
        radius = math.pow(math.pow(deltaX,2) + math.pow(deltaY,2),.5)*1000#how?
        return radius
...
def zoom(factor):
    global CONSTANT_RATIO
    locs = []
    for index in range(len(objs)):
        locs.append([0,0])
    for index in range(len(objs)):
        obj = objs[index]
        locs[index][0] = 400 +int((obj.locX-400)*factor)
        locs[index][1] = 240 +int((obj.locY-240)*factor)
    
    CONSTANT_RATIO = CONSTANT_RATIO / factor
    return locs
...
def run():
    ...
    while True:
        touch_point = touchscreen_finger_points_multitouch()
        if len(touch_point) != 0:
            # get the x and y coordinates of the touch
            if len(touch_point) == 1:
                x = touch_point[0]['x']
                y = touch_point[0]['y']
                if x > 710 and y > 430:
                    add()
#                    *********************************************************************************************************
            if len(touch_point) == 2 and first:
                print int(touch_point[0]['x']/100)
                locs = zoom(2)
...
#                    first = False
#                    sTime = time.time()
#                    xInit1  = touch_point[0]['x']
#                    yInit1  = touch_point[0]['y']
#                    xInit2  = touch_point[1]['x']
#                    yInit2  = touch_point[1]['y']
#                    deltaX = abs(xInit1-xInit2)
#                    deltaY = abs(yInit1-yInit2)
#                    dInit = math.pow(math.pow(deltaX,2) + math.pow(deltaY,2),.5)
#                else:
#                    x1  = touch_point[0]['x']
#                    y1 = touch_point[0]['y']
#                    x2  = touch_point[1]['x']
...
#                    deltaX = abs(x1-x2)
#                    deltaY = abs(y1-y2)
#                    dFin = math.pow(math.pow(deltaX,2) + math.pow(deltaY,2),.5)
#                    zoom(dFin/dInit)
#            else:
